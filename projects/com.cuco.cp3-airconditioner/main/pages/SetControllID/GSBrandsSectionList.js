...
export default class GSBrandsSectionListPage extends SafeAreaBaseContainer {
  ...
  componentDidMount() {
    ...
    /** * let sections = [
         ...
         */
    GSIRAirconditionerMgr.iracBrands()
      .then((res) => {
        if (res.code == 0) {
          console.log('=======iracBrands res');
          ...
          sortKeys.map((key) => {
            try {
              sortBrandSctions.push(
                brandSections.filter((val) => val.key === key)[0]
              );
            } catch (error) {}
          });
  ...
  onGotoMatchPage = (item) => {
    ...
    GSIRAirconditionerMgr.iracMatchCodeInfo(item.id)
      .then((res) => {
  ...
  gsRender() {
    ...
    let keys = brandSections.map((val) => {
      return val.key;
    });
    return (
