...
import { GSSetSpecResone, GSSetSpecResult, GSSetSpecResults, GSSetSpecSuccess } from "../../../constants/protocol/spec";
  ...
  constructor() {
    ...
    switchAndDuration = async() => {
      ...
      if (res.success) {
        ...
        values.map((val) => {
          ...
          if (!!success && siid == GSSpec.siid8.status.siid && piid == GSSpec.siid8.status.piid) {
            results['switchOn'] = value;
          } else if (!!success && siid == GSSpec.siid8.keep_time.siid && piid == GSSpec.siid8.keep_time.piid) {
    ...
    specSetSwithAndDuraton = (swittchOn, duration) => {
      ...
      let promise = new Promise((resolve, reject) => {
        const { siid: s0, piid: p0 } = GSSpec.siid8.status; // 开关  
        const { siid: s1, piid: p1 } = GSSpec.siid8.keep_time; // 持续时间       
        let params = [{ did: GSDeivce.did, siid: s0, piid: p0, value: swittchOn }, 
