...
export default class HomePage extends SafeAreaBaseContainer {
  constructor(props) {
    ...
    this.state = {
      ...
      isVoiceDevice: Device.isVoiceDevice
    };
  }
  ...
  UNSAFE_componentWillMount() {
    ...
    DateManagerMgr.getDayPowerNumber().then((res) => {
      ...
      console.log('getDayPowerNumber', err);
    });
    DateManagerMgr.getAllMonthPowerNumber().then((res) => {
      console.log('>>>>>>>>>>>>>>>>>>>>>>>>>::::::', res);
      let totalMonthPower = DateManagerMgr.CalculateTotalPower(res);
      this.setState({ totalMonthElectricity: totalMonthPower.toString() });
      // console.log("month=========================month:================", JSON.stringify(res));
    }).catch((err) => {
      console.log('getAllMonthPowerNumber', err);
    });
    ...
    LocalStorageMgr.get(
      ...
      .catch((err) => {
        console.log('LocalStorageMgr getDayPowerNumber', err);
      });
    this.addListener();
  }
  ...
  addListener() {
    ...
    Device.getDeviceWifi()
      .subscribeMessages('prop.power', 'prop.7.1', 'prop.7.6')
      .then((subcription) => {
    ...
    this.mDeviceReceivedMessages =
      DeviceEvent.deviceReceivedMessages.addListener((device, map, data) => {
  ...
  isCurrentComfyMode = () => {
    ...
    let value =
      mode === ACMode.cold.value &&
      tempValue === 28 &&
      windValue === WIND.auto.value;
  ...
  // 小爱语控
  onPressAiControl = () => {
    Host.ui.openProductBaikeWebPage(
      `https://home.mi.com/webapp/content/baike/product/index.html?model=${CucoDevice.model}#/aiCase`
    );
  };
  ...
  render() {
    ...
    // 设置导航栏
    let subtitle =
      onDelayControlVal.value !== undefined && onDelayControlVal.value !== ''
        ? onDelayControlVal.value
        : undefined;
    ...
    // 设置背景view
    let bg = powerOn
      ? { color: '#57B3E7', opacity: 1.0, color1: '#57B3E7', opacity1: 0.0 }
      : { color: '#324F60', opacity: 1.0, color1: '#324F60', opacity1: 0.0 };
    if (GSSystem.isDarkMode()) {
      // 黑暗模式
      bg = powerOn
        ? { color: '#003934', opacity: 1.0, color1: '#000000', opacity1: 1.0 }
        : { color: '#324F60', opacity: 0.2, color1: '#000000', opacity1: 1.0 };
    }
    this.setDefaultGradientBackground(
      bg.color,
      bg.opacity,
      bg.color1,
      bg.opacity1
  ...
  gsRender() {
    ...
    let iSpeeds =
      !speeds || speeds.length < 2
        ? HomePagePannelControl.defaultWindTypes
        : speeds;

    // mode 空调模式
    let currentModes = HomePagePannelControl.getModesFromFeatures();
    let iModes =
      currentModes.length > 2
        ? currentModes
        : HomePagePannelControl.defaultModes;
      ...
      <View style={styles.container}>
        ...
        <View style={styles.stateContiner}>
          ...
          <Text style={styles.stateText}>
            {ExchangeTxtPosition(GSLocalize('acid15'), windValueName)}
          </Text>
          <Text style={styles.stateText}>{'｜'}</Text>
        ...
        {/* 今日电量，当月电量，当前功率 */}
        <TouchableOpacity
          style={styles.consumptionCellContainer}
          onPress={() => {
            this.onPressStatistics();
          }}
        >
          <View style={styles.consumptionCellContainerTop}>
            <View style={styles.consumptionCell}>
              <Text style={styles.consumptionCellValue}>
                {this.state.ElectricQuantity}
              </Text>
              <Text style={styles.consumptionCellDes}>
                {GSLocalize('acid42')}
              </Text>
            </View>
            <View
            ...
            <View style={styles.consumptionCell}>
              <Text style={styles.consumptionCellValue}>
                {this.state.totalMonthElectricity}
              </Text>
              <Text style={styles.consumptionCellDes}>
                {GSLocalize('acid45')}
              </Text>
          ...
          <View>
            <Text style={styles.consumptionCellMore}>
              {GSLocalize('viewMore')}
            </Text>
          </View>
        </TouchableOpacity>
        ...
        <View style={styles.temControlContainer}>
          ...
          >
            ...
            >
              ...
              >
                <Image
                  source={GSImage.temperatureMinus}
                  style={{ ...IconStyles.size42, marginLeft: 3 }}
                />
              </TouchableOpacity>
              <View
                style={{ width: 2, height: 48, backgroundColor: 'white' }}
              />
        ...
        {swingType == 2 && (
          <View style={styles.acLightContainer}>
            <MHCard
              ...
              cardType={MHCard.CARD_TYPE.SWITCH}
              cardRadiusType={MHCard.CARD_RADIUS_TYPE.ALL}
              onValueChange={() => this.onPressSwingOn(!swingOn)}
              switchValue={swingOn}
              tintColor="#E5E5E5"
              onTintColor="#57B3E7"
              showShadow={true}
        ...
        <View style={styles.acLightContainer}>
          <MHCard
            ...
            showShadow={true}
            marginTop={0}
            icon={isVoiceDevice ? GSImage.xiaoaiControlOn : GSImage.xiaoaiControl}
            iconStyle={{ ...IconStyles.size42 }}
            cardStyle={{
              borderRadius: 12,
              width: contentWidth,
            ...
            onPress={() => {
              this.onPressAiControl();
            }}
          />
          {(!isVoiceDevice) && ACTurnOffMask}
        </View>
        {/* 安睡模式，定时，延迟关空调 */}
        <View style={styles.sleepModeContainer}>
          ...
          <View>
            <MHCard
              ...
              icon={
                onDelayControlVal.on ? GSImage.DelayCloseOn : GSImage.DelayClose
              }
              cardStyle={{
                width: contentWidth,
