 ...
 * 周电量统计
 */
import { ChoiceDialog, ListItem, ListItemWithSwitch, MHDatePicker, NavigationBar, Separator } from "mhui-rn";
import React from "react";
import { StyleSheet, Text, Touchable, TouchableOpacity, TouchableWithoutFeedback, View, PanResponder, Animated, TabBarIOS } from "react-native";
...
export default class SleepModePage extends SafeAreaBaseContainer {
  ...
  calculateTempForHeight = (height) => {
    ...
    let temp = parseInt(((height - minTempH) / (maxTempH - minTempH) * (max - min)) + min);
    if (temp < min) {
      temp = min;
    } else if (temp > max) {
      temp = max;
  ...
  createPanResponder = (index) => {
    return PanResponder.create({
      ...
      onPanResponderMove: (e, g) => {
        ...
        let min = minTempH;
        let max = maxTempH;
        const { controlBarIndex, controlBarH0, controlBarH1, controlBarH2 } = this.state;
  ...
  /**
     ...
     */
  render() {
    const { sleepDuringPickerType, showWindPicker, showAfterSleepPicker, showDelayClosePicker, on } = this.state;
    // 设置导航栏
  ...
  gsRender() {
    ...
    let startStr_30 = SleepModePageControl.changeMintesToTimeStr((startTime + 30) % 1440);
    let endStr_30 = SleepModePageControl.changeMintesToTimeStr(endTime < 30 ? endTime + 1440 - 30 : endTime - 30);
    return <View style={styles.container}>
      {/* 安睡模式 */}
      <ListItemWithSwitch
        title={GSLocalize("acid67")}
        subtitle={GSLocalize("acid71")}
        containerStyle={{ width: MainScreen.width, height: 72, backgroundColor: 'white' }}
        titleStyle={{ textAlign: 'left', fontSize: 16, color: '#000000', fontWeight: GSFont.Semibold }}
        subtitleStyle={{ textAlign: 'left', fontSize: 13, color: '#999999', fontWeight: GSFont.Regular }}
        onValueChange={(val) => {
          this.enableLoadingActivity(true);
      ...
      {!!on && <ListItem
        ...
        containerStyle={{ width: MainScreen.width, height: 72, backgroundColor: 'white' }}
        titleStyle={{ textAlign: 'left', fontSize: 16, color: '#000000', fontWeight: GSFont.Semibold }}
        valueStyle={{ textAlign: 'left', fontSize: 13, color: '#999999', fontWeight: GSFont.Regular, width: 100 }}
        onPress={() => {
          this.setState({
            showWindPicker: true
          });
      ...
      {/* 扫风 */
        !!on && !!isHadSwing && <ListItemWithSwitch
      ...
      {!!on && <ListItem
        ...
        subtitle={afterSleepState == 2 ? `${ afterSleepDoTitles[afterSleepState] } ${ delayTime } ${ GSLocalize("acid94") }` : afterSleepDoTitles[afterSleepState]}
        containerStyle={{ width: MainScreen.width, height: 72, backgroundColor: 'white' }}
        titleStyle={{ textAlign: 'left', fontSize: 16, color: '#000000', fontWeight: GSFont.Semibold }}
        subtitleStyle={{ textAlign: 'left', fontSize: 13, color: '#999999', fontWeight: GSFont.Regular }}
        onPress={(val) => {
      ...
      />}
      {/* 横线 */}
      {!!on && <Text style={{ marginLeft: 27, width: CW, lineHeight: 1, marginTop: 20, backgroundColor: '#E5E5E5', color: '#E5E5E5' }}>-</Text>}
      {!!on && <View style={{ marginLeft: 27, width: CW, height: 236, backgroundColor: 'white' }}>
        {/* 头部 */}
        <View style={{ width: '100%', height: 42, flexDirection: 'row', alignItems: 'flex-end' }}>
          <Text style={{ width: '25%', textAlign: 'center', lineHeight: 14, marginBottom: 8, fontWeight: GSFont.Regular, fontSize: 10, color: '#7F7F7F' }}>{`${ beginTemp }°C`}</Text>
          <Text style={{ width: '50%', textAlign: 'center', lineHeight: 14, marginBottom: 8, fontWeight: GSFont.Regular, fontSize: 10, color: '#7F7F7F' }}>{`${ middleTemp }°C`}</Text>
          <Text style={{ width: '25%', textAlign: 'center', lineHeight: 14, marginBottom: 8, fontWeight: GSFont.Regular, fontSize: 10, color: '#7F7F7F' }}>{`${ endTemp }°C`}</Text>
        </View>
        {/* 中间 */}
        <View style={{ width: '100%', height: maxTempH, flexDirection: 'row', backgroundColor: '#F7F7F7' }}>
          ...
          {/* 手势 */}
          {
            <Animated.View style={{ position: 'absolute', width: '100%', height: '100%', backgroundColor: 'transparent' }}
        ...
        <View style={{ width: '100%', height: 34 }}>
          <Text style={{ position: 'absolute', marginLeft: 0, width: 30, textAlign: 'left', lineHeight: 14, marginTop: 8, fontWeight: GSFont.Regular, fontSize: 10, color: '#999999' }}>
            {startStr}
          </Text>
          <Text style={{ position: 'absolute', marginLeft: CW * 0.25 - 15, width: 30, textAlign: 'center', lineHeight: 14, marginTop: 8, fontWeight: GSFont.Regular, fontSize: 10, color: '#999999' }}>
            {startStr_30}
          </Text>
          <Text style={{ position: 'absolute', marginLeft: CW * 0.25 - 15 + CW * 0.5, textAlign: 'center', lineHeight: 14, marginTop: 8, fontWeight: GSFont.Regular, fontSize: 10, color: '#999999' }}>
            {endStr_30}
          </Text>
          <Text style={{ position: 'absolute', marginLeft: CW - 30, textAlign: 'right', width: 30, lineHeight: 14, marginTop: 8, fontWeight: GSFont.Regular, fontSize: 10, color: '#999999' }}>
            {endStr}
          </Text>
        </View>
      </View>}
      {/* 虚线 */}
      {
        !!on && GSDottedLine({ width: MainScreen.width - 54, lineColor: '#e5e5e5', style: { marginTop: 10, marginLeft: 27 } })
      }
      {!!on && <Text style={{ marginBottom: 60, color: '#999999', lineHeight: 16, width: CW, textAlign: 'left', fontSize: 12, fontWeight: GSFont.Regular, marginTop: 20, marginLeft: 27 }}>
        {GSLocalize("acid8")}
      </Text>}
      {
        // toast: 时间间隔不能小于多少
        !!showToast && <View style = {{ position: 'absolute', borderRadius: 10, height: 40, width: MainScreen.width - 120, marginLeft: 60, backgroundColor: "rgba(0,0,0,0.45)", marginTop: MainScreen.centerY(40) - 80, justifyContent: 'center' }}>
          <Text style = {{ textAlign: 'center', lineHeight: 25, color: 'white' }}>
            {GSLocalize('acid102')}
          </Text>
        </View>
      }
  ...
  onConfirmSleepDuringSetting = () => {
    const { willStartTime, willEndTime, startTime, endTime, willRepeats, repeats } = this.state;
  ...
  sleepDuringChoiceView = () => {
    const { willStartTime, willEndTime, startTime, endTime, selectedReatItemIndex, repeats, willRepeats } = this.state;
    ...
    return <GSDialogPage
      height={331}
      onHide={
        this.onCancelSleepDuringSetting
      }
      customView={
        <View>
          <ListItem
          ...
          <ListItem
            ...
            showSeparator={false}
          />
          <View style = {{ width: '100%', height: 44, flexDirection: 'row', justifyContent: 'center' }}>
            <TouchableOpacity style = {styles.leftButtonBgStyle} onPress = {
              this.onCancelSleepDuringSetting
            }>
              <Text style = {styles.cancelButtonText}>{GSLocalize('acid54')}</Text>
            </TouchableOpacity>
            <TouchableOpacity style = {styles.rightButtonBgStyle} onPress = {
              this.onConfirmSleepDuringSetting
            }>
              <Text style = {styles.confirmButtonText}>{GSLocalize('acid51')}</Text>
            </TouchableOpacity>
          </View>
        </View>
      } />;
  ...
  repeatChoiceView = () => {
    ...
    let contentVieW = <View>
      <SleepDuringItemCell title={GSLocalize("acid78")} onSelected={selectedReatItemIndex == 0} onPress={() => {
        this.onSelectRepeatValue(0); // 每天
      }} />
      <SleepDuringItemCell title={GSLocalize("acid77")} onSelected={selectedReatItemIndex == 1} onPress={() => {
        this.onSelectRepeatValue(1); // 工作日
      }} />
      <SleepDuringItemCell title={GSLocalize("acid79")} onSelected={selectedReatItemIndex == 2} onPress={() => {
        this.onSelectRepeatValue(2); // 周末
      }} />
      <ListItem title={GSLocalize("acid80")} containerStyle={{ marginTop: 0, width: MainScreen.width, height: 52, backgroundColor: 'white' }}
        titleStyle={{ textAlign: 'left', fontSize: 16, color: '#000000', fontWeight: GSFont.Regular, height: 20 }}
        onPress={(val) => {
  ...
  benginTimeView = () => {
    return <MHDatePicker
      ...
      onSelect={(res) => {
        ...
        this.setState({
          sleepDuringPickerType: 0, // -1
          willStartTime: SleepModePageControl.changeArrayToMinutes(res.rawArray)
        });
      }}
      onDismiss={(_) => {
        this.setState({
  ...
  endTimeView = () => {
    return <MHDatePicker
      ...
      onSelect={(res) => {
        this.setState({
          sleepDuringPickerType: 0, // -1
          willEndTime: SleepModePageControl.changeArrayToMinutes(res.rawArray)
        });
      }}
      onDismiss={(_) => {
        this.setState({
  ...
  // 显示 风速picker
  windSpeedTitles = () => [GSLocalize("acid16"), GSLocalize("acid92"), GSLocalize("acid91"), GSLocalize("acid90")];
  windPickerView = () => {
    ...
    return <ChoiceDialog
      visible={true}
      title={GSLocalize("acid15")}
      options={speeds.map((val) => { return { title: this.windSpeedTitles()[val] }; })}
      canDismiss={() => { }}
  ...
  afterSleepView = () => {
    ...
    let contentVieW = <View>
      <SleepDuringItemCell title={afterSleepDoTitles[1]} onSelected={afterSleepState == 1} onPress={() => {
        this.onSelectAfterSleep(1);
      }} />
      <SleepDuringItemCell title={afterSleepDoTitles[0]} onSelected={afterSleepState == 0} onPress={() => {
  ...
  delayColseTimePickerView = () => {
    return <MHDatePicker
      ...
      min={['1']}
      max={['59']}
      onSelect={(res) => { // 点击确认，onDismiss 也会执行。 很怪异。 需要加个 hadOnConfirmDelayTime 屏蔽下。
...
const afterSleepDoTitles = [GSLocalize("acid11"), GSLocalize("acid10"), GSLocalize("acid93")];
