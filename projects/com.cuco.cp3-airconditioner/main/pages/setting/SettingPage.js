...
export default class SettingPage extends SafeAreaBaseContainer {
  ...
  componentDidMount() {
    ...
    this.viewAppearListener = PackageEvent.packageViewWillAppear.addListener(() => {
      console.log(" ========== viewAppearListener  componentDidMount   viewAppearListener ======");
      this.getBrand(GSDeivce.brand.id);
      this.getBrandAndPos();
      this.getIndicatorLightInfo();
  ...
  getBrand(brandId) { // 获取品牌
    ...
    LocalStorageMgr.get(`brand_${ Device.deviceID }_${ brandId }_${ lan }`).then((ress) => {
        ...
        GSIRAirconditionerMgr.getIrBrand(brandId).then((res) => {
          ...
          if (res.code == 0 && !this.unmount) {
            if (lan == "zh") {
              ...
              LocalStorageMgr.set(`brand_${ Device.deviceID }_${ brandId }_${ lan }`, { "brand_name": res.result.name });
              this.setState({
                "brand_name": res.result.name
              });
            } else {
              GSDeivce.brand.name = res.result.en_name;
              LocalStorageMgr.set(`brand_${ Device.deviceID }_${ brandId }_${ lan }`, { "brand_name": res.result.en_name });
              this.setState({
  ...
  getBrandAndPos() {
    ...
    let airSelectedPos = "";
    LocalStorageMgr.get("airSelectedInfo").then((res) => {
  ...
  JumpSettingCode() {
    ...
    navigatePushPage(this, "SetControllID", { from: "Setting", style: "Coding", Title: GSLocalize("acid30") });
  }
  getIndicatorLightInfo() {
    let params = [
      { did: GSDeivce.did, siid: GSSpec.siid10.status.siid, piid: GSSpec.siid10.status.piid },
      { did: GSDeivce.did, siid: GSSpec.siid10.model.siid, piid: GSSpec.siid10.model.piid },
      { did: GSDeivce.did, siid: GSSpec.siid10.start_time.siid, piid: GSSpec.siid10.start_time.piid },
      { did: GSDeivce.did, siid: GSSpec.siid10.end_time.siid, piid: GSSpec.siid10.end_time.piid }
    ];
  ...
  lightStatusInfo(setStates) {
    ...
    if (setStates["lightSwitch"]) {
      if (setStates["doNotDisturbModel"]) {
        let startTimeMinute = this.AllMinuteToHourAndMinute(setStates["startTimeMinute"]);
        let endTimeMinute = this.AllMinuteToHourAndMinute(setStates["endTimeMinute"]);
        if (setStates["startTimeMinute"] < setStates["endTimeMinute"]) { // 同一天
          info = `${ startTimeMinute.join(":") }-${ endTimeMinute.join(":") } ${ GSLocalize("acid127") }`;
        } else { // 第二天
          info = `${ startTimeMinute.join(":") }-${ GSLocalize("acid56") }${ endTimeMinute.join(":") } ${ GSLocalize("acid127") }`;
        }
      } else {
  ...
  render() {
    ...
    this.setDefaultNavibar({
      ...
      onback: () => { navigatePushPage(this, "Home"); }
    });
    ...
    return (
      <View style={styles.container}>
        ...
        >
          ...
          <View style={{ backgroundColor: 'transparent', height: 32, justifyContent: 'center', paddingLeft: SdkStyles.common.padding }}>
          ...
          <ListItem
            ...
            showSeparator={false}
            onPress={() => {
              navigatePushPage(this, "SetControllID", { from: "Setting", style: "setModel", Title: GSLocalize('acid30') });
            }}
          />
          <ListItem
          ...
          <View style={{ height: 40, justifyContent: 'center', alignItems: 'center', backgroundColor: 'transparent', paddingLeft: 27, paddingRight: 27 }}>
            <View style={{ height: 1, backgroundColor: '#E5E5E5', width: '100%', flexDirection: 'row' }}></View>
          </View>
